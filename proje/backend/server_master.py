# server_master.py (düzeltilmiş — tüm dosya)
import os
import socket
import struct
import hashlib
...
HOST = "127.0.0.1"
PORT = 5000
THRESHOLD = 0.6      # ses için eşleşme eşiği
NEWS_THRESHOLD = 0.6 # kullanılmıyor ama bırakıldı
...
def unpack_hashes(buf):
    """
    Tek bir pack_hashes buffer'ından hash listesi döndürür.
    Eğer buf formatı hatalıysa ValueError atar.
    """
    if len(buf) < 4:
        raise ValueError("Bozuk veri: uzunluk alanı yok.")
    n = struct.unpack(">I", buf[:4])[0]
    expected = 4 + 32 * n
    if len(buf) != expected:
        raise ValueError("Bozuk veri: uzunluk uyuşmuyor.")
    hashes = [buf[4+i*32:4+(i+1)*32] for i in range(n)]
    return hashes

def unpack_two_packs(buf):
    """
    İki tane pack_hashes()'ın ard arda konduğu buffer'ı parse eder.
    Returns (listA, listB)
    """
    if len(buf) < 4:
...
# --- Haber Enrollment Kalıcı Dosyası (PERSISTENT) ---
...
# --- Yardımcı: ham içerikten 8-byte bloklar için sha256 üret ---
...
def handle_client(conn):
    ...
    try:
        if msg_type == 5:
            ...
            try:
                ...
                # Eğer paketlenmiş hash beklenmiyorsa (bazı client'lar doğrudan dosya içerik göndermiş olabilir),
    ...
    if msg_type == 1:
        ...
        print(f"[VERIFY] {same}/{m} eşleşti (ratio={ratio:.3f}) -> {'OK' if ok else 'FAIL'}")

    elif msg_type == 3:
        # Haber enroll: hash_list burda list of hashes olmalı (fallback ile oluştuysa da olur)
        ...
        if not enrolled:
            ...
            print("[NEWS VERIFY] Enrollment yok veya kayıt dosyası boş/bozuk.")
            return

        # Birebir eşleşme kontrolü (exact)
        if hash_list == enrolled:
            ratio = 1.0
            ok = True
        else:
            ratio = 0.0
        ...
        send_packet(conn, (b"\x01" if ok else b"\x00") + msg)
        print(f"[NEWS VERIFY] incoming={len(hash_list)} enrolled={len(enrolled)} -> {'SAME' if ok else 'DIFFERENT'}")
...
def start_server(host=HOST, port=PORT):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        s.bind((host, port))
        s.listen(1)
        print(f"[SERVER] {host}:{port} dinlemede. CTRL+C ile çık.")
        try:
            while True:
                conn, addr = s.accept()
                with conn:
